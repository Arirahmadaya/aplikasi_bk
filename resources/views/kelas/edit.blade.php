...
<form action='{{ url('kelas/'.$data->id)}}' method='post'>
    ...
    <div class="my-3 p-3 bg-body rounded shadow-sm">
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                <input type="number" class="form-control" name='jumlah_siswa' value="{{$data->jumlah_siswa}}" id="jumlah_siswa">
            </div>
        </div>
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                <select name="id_wali_kelas" id="id_wali_kelas" class="form-control">
                    ...
                    <option {{ $item->id == $data->id_wali_kelas? 'selected' : '' }} value="{{$item->id}}">{{$item->nama}}</option>        
                    @endforeach
                </select>
            </div>
        </div>
